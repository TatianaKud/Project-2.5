class PageChanger {
    ...
    changeAttr(selectorToChange, attrToChange, sourceInputName) {
      ...
      if (selector.getAttribute('href').includes('mailto:')) {
        selector.setAttribute(attrToChange, `mailto:${this.getValue(sourceInputName)}`);
      } else {
    ...
    changeDate(selectorToChange, sourceInputName) {
      document.querySelector(selectorToChange).textContent = this.getValue(sourceInputName).split('-').reverse().join('.');
    }
  }
  ...
  const photos = {
    bombardier : {
                    ...
                    imageUrl: 'https://pictures.s3.yandex.net/html-forms/BOMBARDIER.jpg',
                    photoYear: '2017',
                    creditLink: 'https://www.flickr.com/photos/sloppyperfectionist/',
                    ccLink: 'https://creativecommons.org/licenses/by/2.0/legalcode',
                    ccType: 'CC BY-SA 2.0',
                    ccImg: 'https://pictures.s3.yandex.net/html-forms/BOMBARDIER_CC.svg',
                    ccHeight: '10px',
                  },
    hobbit : {
                photoName: 'Smaug!',
                photoAuthor: 'Alan Wilson',
                imageUrl: 'https://pictures.s3.yandex.net/html-forms/HOBBIT.jpg',
                photoYear: '2014',
                creditLink: 'https://www.flickr.com/photos/ajw1970/14323036295/',
                ccLink: 'https://creativecommons.org/licenses/by/2.0/legalcode',
                ccType: 'CC BY-SA 2.0',
                ccImg: 'https://pictures.s3.yandex.net/html-forms/HOBBIT_CC.svg',
                ccHeight: '10px'
              },
    tiger : {
                photoName: 'EI-XLD',
                photoAuthor: 'Papas Dos',
                imageUrl: 'https://pictures.s3.yandex.net/html-forms/TIGER.jpg',
                creditLink: 'https://www.flickr.com/photos/papasdos/34150356073/',
                photoYear: '2017',
                ccLink: 'https://creativecommons.org/licenses/by/2.0/legalcode',
                ccType: 'CC BY 2.0',
                ccImg: 'https://pictures.s3.yandex.net/html-forms/TIGER_CC.svg',
                ccHeight: '18px',
              },
    connected : {
                  photoName: 'N709JB Fly-Fi',
                  photoAuthor: 'Dave Montiverdi',
                  imageUrl: 'https://pictures.s3.yandex.net/html-forms/CONNECTED.jpg',
                  photoYear: '2018',
                  ...
                  ccImg: 'https://pictures.s3.yandex.net/html-forms/CONNECTED_CC.svg',
                  ccHeight: '10px',
                },
  }
  
  function createCredit({creditLink, photoAuthor, photoName, photoYear, ccLink, ccType, ccImg, ccHeight}) {
    return `Фото: <a class="content__link" href="${creditLink}" target="_blank">${photoAuthor}</a> "${photoName}" © ${photoYear}, &nbsp;&nbsp; <a class="content__link" href="${ccLink} target="_blank">${ccType}</a> &nbsp;&nbsp; <img src=${ccImg} height=${ccHeight}px alt=${photoName}>`
  }
  ...
  document.querySelector('.form__admin').addEventListener('submit', function(e) {
    ...
    page.changeStyle('.content__heading', 'font-size', page.getValue('heading-font-size')+'px');
    page.changeStyle('.content__text', 'font-size', page.getValue('text-font-size')+'px');
    ...
    switch(page.getValue('image')) {
      ...
      case 'hobbit':
          ...
          captionToChange.innerHTML = 'Дракон только что приземлился в Лос-Анджелесе. ' + createCredit(photos.hobbit);
          break;
      case 'tiger':
          imgOnPageToChange.setAttribute('src', photos.tiger.imageUrl);
          imgOnPageToChange.setAttribute('alt', photos.tiger.photoName);
          captionToChange.innerHTML = '«Тигролёт» над международным аэропортом «Внуково». ' + createCredit(photos.tiger);
          break;
      case 'connected':
          ...
          captionToChange.innerHTML = 'В аэропорту Вестчестер, штат Нью-Йорк. ' + createCredit(photos.connected);
          break;
      default:
          imgOnPageToChange.setAttribute('src', photos.bombardier.imageUrl);
          imgOnPageToChange.setAttribute('alt', photos.bombardier.photoName);
    ...
    document.querySelectorAll('.content__link').forEach(item => item.style.color = page.getValue('text-color'));
  
    page.changeStyle('.content__text', 'width', page.getValue('content-width')+'%');
